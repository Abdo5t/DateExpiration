...
Window.clearcolor = (0.95, 0.95, 0.95, 1)   # Transparent background 
...
def calculer_date_expiration(nom_produit: str, ajouter_jour: int = 0) -> str:
    ...
    mois_latins = ["JAN", "FEV", "MAR", "AVR", "MAI", "JUN", "JUL", "AOU", "SEP", "OCT", "NOV", "DEC"]
...
#Spinner chose products and colors 
...
#If the products is LHP move to add 1 d
...
#Botton to Annuler 
  ...
  #add 1 day and calculate the result 
        resultat = calculer_date_expiration(produit, 1)
        self.result_label.text = resultat
        self.show_product_image(produit)

        #Close the windows after confirmation 
        self.popup.dismiss()

    def on_cancel_add_day(self):
        #Annuler 1 day adding
        self.plus_one_spinner.text = "+0"
        self.result_label.text = ""

   #Close the window for confirmation after confirmation 
        self.popup.dismiss()

    def show_product_image(self, produit):
        #Delete the last picture if was it
        
        self.image_widget.clear_widgets()
        
#Add new picture for new product 
