...
class DateExpirationApp(App):
    def build(self):
        ...
        # Résultat (texte + image)
        self.resultat_layout = BoxLayout(
        ...
        self.resultat_layout.add_widget(self.resultat_label)
        self.resultat_layout.add_widget(self.resultat_image)

        principal.add_widget(self.resultat_layout)
    ...
    def calculer_date(self, instance):
        ...
        if produit == "LHP" and self.ajouter_jour_spinner.text == "Oui":
            ...
            self.afficher_resultat(resultat, produit)
    ...
    def confirmer_ajout_jour(self):
        ...
        def on_oui(instance):
            ...
            self.afficher_resultat(resultat, self.produit_spinner.text)
    ...
    def afficher_resultat(self, texte, produit):
        self.resultat_label.text = texte

        images_produits = {
            "LHP": "assets/LHP.png",
            "LBEN": "assets/LBEN.png",
            "LAF": "assets/LAF.png"
        }

        self.resultat_image.source = images_produits.get(produit, '')
        self.resultat_image.reload()

        # Animations Fade In
        self.resultat_label.opacity = 0
        self.resultat_image.opacity = 0

        anim1 = Animation(opacity=1, duration=0.5)
        anim2 = Animation(opacity=1, duration=0.5)

        anim1.start(self.resultat_label)
        anim2.start(self.resultat_image)
